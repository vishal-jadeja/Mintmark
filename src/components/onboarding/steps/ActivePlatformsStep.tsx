...
import { useUpdateActivePlatforms, useUpsertPlatformInstruction } from "@/lib/queries/settings"
...
export function ActivePlatformsStep() {
  ...
  const setActivePlatforms = useOnboardingStore((s) => s.setActivePlatforms)
  const initialPlatforms = useOnboardingStore((s) => s.activePlatforms)

  const [selected, setSelected] = useState<Set<PlatformId>>(
    () => new Set(initialPlatforms.filter((p): p is PlatformId => PLATFORMS.some((pl) => pl.id === p)))
  )
  ...
  async function persistAndAdvance(platforms: string[]) {
    await updateActivePlatforms.mutateAsync({ active_platforms: platforms })
    for (const platform of platforms) {
      const inst = instructions[platform as PlatformId]
      if (inst?.tone || inst?.instruction_text || inst?.format_rules) {
    ...
    <div className="flex flex-col gap-5">
      ...
      <div>
        ...
        <p className="mt-1 font-body text-sm text-muted-foreground">
          Content Studio only generates drafts for these. You can change this any time in Settings.
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Icon */}
                <span style={{ color: isSelected ? "var(--mm-gold-400)" : "var(--muted-foreground)" }}>
                  <Icon />
                </span>
              ...
              {/* Expandable AI instruction fields */}
              <AnimatePresence initial={false}>
                {isSelected && !isCollapsed && (
                  ...
                  >
                    ...
                    >
                      <p className="pt-3 font-body text-xs" style={{ color: "var(--muted-foreground)" }}>
      ...
      <div className="flex flex-col items-center gap-3 mt-1">
        ...
        >
          {loading ? <span className="animate-pulse">Saving…</span> : "Continue →"}
        </button>
        ...
        >
          Skip — I don&apos;t post anywhere yet
        </button>
